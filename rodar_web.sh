...
if [ ! -f "$PASSWORDS_FILE" ] || [ ! -s "$PASSWORDS_FILE" ]; then
    ...
    echo '{"belle": "admin"}' > "$PASSWORDS_FILE"
fi

# Configura o usuario belle como admin no airflow.cfg
if grep -q "^simple_auth_manager_users =" "$CFG_FILE"; then
    ...
    sed -i 's/^simple_auth_manager_users =.*/simple_auth_manager_users = belle:admin/' "$CFG_FILE"
else
    # Se nao existe ou esta comentada, adiciona/substitui
    if grep -q "simple_auth_manager_users =" "$CFG_FILE"; then
        sed -i 's/.*simple_auth_manager_users =.*/simple_auth_manager_users = belle:admin/' "$CFG_FILE"
    else
        echo "simple_auth_manager_users = belle:admin" >> "$CFG_FILE"
    fi
fi
